import turtle

#turtle is an important graphics import

#Sets up the window

wn = turtle.Screen()
wn.title("Pong The Game")
wn.bgcolor("black")
wn.setup(width=800, height=600)
wn.tracer(0)

#Adding paddle A
paddle_a = turtle.Turtle()

#Sets animation speed to max for paddle movement
paddle_a.speed(0)

paddle_a.shape("square")
paddle_a.color("white")
paddle_a.shapesize(stretch_wid=5, stretch_len=1)

#to avoid drawing a line for its movement
paddle_a.penup()

paddle_a.goto(-350,0)

#Adding paddle B
paddle_b = turtle.Turtle()

#Sets animation speed to max for paddle movement
paddle_b.speed(0)

paddle_b.shape("square")
paddle_b.color("white")
paddle_b.shapesize(stretch_wid=5, stretch_len=1)

#to avoid drawing a line for its movement
paddle_b.penup()

paddle_b.goto(350,0)

#Adding Ball

ball = turtle.Turtle()

#Sets animation speed to max for movement
ball.speed(0)

ball.shape("square")
ball.color("White")

#to avoid drawing a line for its movement
ball.penup()

ball.goto(0,0)

#change in ball movement
ball.dx=0.06
ball.dy=0.06

#Scoring
score_a = 0
score_b = 0

# Turtle for Scoring

'''
pen = turtle.Turtle()
pen.speed(0)
pen.color("white")
pen.penup()
pen.hideturtle()
pen.goto(0,260)
pen.write("Player A:  Player B: " , align = "center", font = ("Courier", 24, "normal"))
'''



#Functions for movement
def paddle_a_up():
    y = paddle_a.ycor()
    y += 20
    paddle_a.sety(y)

def paddle_a_down():
    y = paddle_a.ycor()
    y -= 20
    paddle_a.sety(y)

def paddle_b_up():
    y = paddle_b.ycor()
    y += 20
    paddle_b.sety(y)

def paddle_b_down():
    y = paddle_b.ycor()
    y -= 20
    paddle_b.sety(y)

#Keyboard binding
wn.listen()
wn.onkeypress(paddle_a_up, "w")
wn.onkeypress(paddle_a_down, "s")
wn.onkeypress(paddle_b_up, "Up")
wn.onkeypress(paddle_b_down, "Down")

#main game loop

while True:
    #you need to update bc you set tracer to 0 it will not
    #update automatically
    wn.update()

    #move the ball
    ball.setx(ball.xcor() + ball.dx)
    ball.sety(ball.ycor() + ball.dy)


    #Border Checking

    if ball.xcor() > 390:
        ball.goto(0,0)
        ball.dx *= -1
        score_a += 1
        #pen.clear()
        #pen.write("Player A: {} Player B: {}".format(score_a, score_b) , align="center", font=("Courier", 24, "normal"))

    if ball.xcor() < -390:
        ball.goto(0,0)
        ball.dx *= -1
        score_b += 1
        #pen.clear()
        #pen.write("Player A: {} Player B: {}".format(score_a, score_b) , align="center",font=("Courier", 24, "normal"))

    if ball.ycor() < -290:
        ball.sety(-290)
        ball.dy *= -1



    if ball.ycor() > 290:
        ball.sety(290)
        ball.dy *= -1


    #Bounce off Paddle and Ball
    if (ball.xcor() > 340 and ball.xcor() < 350) and (ball.ycor() < paddle_b.ycor() + 50 and ball.ycor() > paddle_b.ycor() - 50):
        ball.setx(340)
        ball.dx *= -1



    if (ball.xcor() < -340 and ball.xcor() > -350) and (ball.ycor() < paddle_a.ycor() + 50 and ball.ycor() > paddle_a.ycor() - 50):
        ball.setx(-340)
        ball.dx *= -1



    # AI Player

    if paddle_b.ycor() < ball.ycor() and abs(paddle_b.ycor() - ball.ycor()) >10:
        paddle_b_up()

    elif paddle_b.ycor() > ball.ycor() and abs(paddle_b.ycor() - ball.ycor()) >10 :
        paddle_b_down()


    if paddle_a.ycor() < ball.ycor() and abs(paddle_a.ycor() - ball.ycor()) >10:
        paddle_a_up()

    elif paddle_a.ycor() > ball.ycor() and abs(paddle_a.ycor() - ball.ycor()) >10 :
        paddle_a_down()



